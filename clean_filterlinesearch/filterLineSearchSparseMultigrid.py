...
class interior_pt:
    ...
    def pKKT_solve(self, X, mu, soc=False):
        ...
        self.multigridHierarchy.constructPreconditioner(Ak[:self.n, :self.n], JkT, Jk, self.problem.n1)

        twogridP = two_grid_action(self.multigridHierarchy.Lfine, self.multigridHierarchy.Lcoarse,\
        ...
        sol, info = spla.gmres(Ak, -rk, tol=lintol, atol=lintol, \
                              M = twogridP, maxiter=maxiter, callback=krylov_convergence.callback)
        ...
        print("GMRES iterations = {0:d}".format(len(krylov_convergence.residuals)))
        """
